...
const ContestProblemPage = () => {
    ...
    let { problemId, id: contestId } = useParams();
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        const fetchProblem = async () => {
            ...
            try {
                ...
                const { data: contestData } = await axiosClient.get(
                    `/contest/${contestId}`
                );
    ...
    const handleSubmitCode = async () => {
        ...
        try {
            const response = await axiosClient.post(
                `/contest/${contestId}/submit-solution/${problemId}`,
                {
                    code: code,
                    language: selectedLanguage,
                }
            ...
            <nav className="sticky top-0 z-50 bg-gray-800/90 backdrop-blur-sm border-b border-gray-700 shadow-2xl px-6 py-3">
                <div className="flex justify-between items-center">
                    ...
                    <div className="font-semibold text-2xl">
                        ...
                        <span className="text-sm font-medium ml-10">
                            {/* {status === "Ended" ? "Contest Finished" : `${status === "Upcoming" ? "Starts in" : "Ends in"}:`} */}
                            <span className="ml-2 text-lg font-semibold text-yellow-400">{timer} <span className="text-slate-200 text-sm ml-1">hrs left</span></span>
                        </span>                        
                    </div>
                            ...
                            <>
                                ...
                                {activeLeftTab === "editorial" && (
                                    <div className="prose max-w-none text-gray-300">
                                        ...
                                        <div className="whitespace-pre-wrap text-sm leading-relaxed">
                                            Editorial content will be available
                                            ...
                                            (lang) => (
                                                <button
                                                    ...
                                                    className={`btn btn-sm ${
                                                        selectedLanguage ===
                                                        lang
                                                            ? "btn-primary bg-gradient-to-r from-blue-500 to-purple-600 border-0"
                                                    ...
                                                    <div className="mt-4 space-y-2">
                                                        {runResult.testCases.map(
                                                            (tc, i) => (
                                                                <div
                                                                    key={i}
                                                                    className="bg-gray-800/50 p-3 rounded text-xs border border-gray-700"
                                                                >
